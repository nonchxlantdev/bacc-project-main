-- 013: OM/admin/coo may delete any unlocked draft (cleanup), and expose
-- a one-shot clean-slate helper for the test workspace.
-- Apply in Supabase → SQL Editor if migrations are applied that way.

-- Allow supervisors to remove drafts that are not yet on file.
  ...
  using (
    ...
    and (
      ...
      or exists (
        ...
        where p.id = auth.uid() and p.role in ('om', 'coo', 'admin')
      )
    )
  );

-- One-shot wipe of filed work (keeps catalogue + staff). Call as:
--   select public.clear_operational_data();
...
-- OM can call it from the portal later; role check is inside the function.
