-- 016: Fix draft delete — BEFORE DELETE must return OLD, not NEW.
-- Returning NEW (null on DELETE) silently cancelled every draft delete,
-- which the client reported as an RLS/permission failure.
...
declare
  ...
  new_core jsonb;
begin
  -- DELETE path: allow unlocked drafts; block locked rows.
  ...
  if old.locked = true then
    ...
    -- Approver reopen: OM/admin/coo may flip reopened_* and rewrite items.
    ...
    -- Legacy OM acknowledge path: status → acknowledged + export path only.
...
-- Keep delete RLS aligned with OM/admin-only (014).
