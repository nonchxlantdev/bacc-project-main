  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-ink sm:text-2xl">Notifications</h1>
          <p className="text-sm text-muted">{unread} unread</p>
        </div>
        <button type="button" onClick={() => markAllRead()} className="rounded-md border border-line/20 px-3 py-2 text-sm">
          Mark all read
        </button>
      </div>
      <ul className="divide-y divide-line/10 overflow-hidden rounded-lg border border-line/10 bg-surface">
        {rows.length === 0 && <li className="px-4 py-8 text-center text-sm text-muted">No notifications.</li>}
        {rows.map((row) => (
          <li key={row.id} className={`px-4 py-3 ${row.read_at ? 'bg-surface' : 'bg-primary/5'}`}>
            <Link
